...
class WorkoutManager: NSObject, ObservableObject {
  ...
  override init() {
    ...
    // Subscribe to changes in workoutState from WatchConnectivityManager
    workoutStateSubscriber = WatchConnectivityManager.shared.$workoutState.sink { [weak self] state in
      // Handle changes to workoutState
      self?.handleWorkoutStateChange(state)
    }
  }
  
  private func handleWorkoutStateChange(_ state: String?) {
    // Handle workoutState changes here
    print("New workout state received: \(state ?? "Unknown")")
    switch state {
    case WorkoutState.inProgress.rawValue:
      resume()
    case WorkoutState.completed.rawValue:
      endWorkout()
    case WorkoutState.paused.rawValue:
      pause()
    case WorkoutState.canceled.rawValue:
      endWorkout()
    default:
      return
    }
  ...
  func startWorkout(workoutType: HKWorkoutActivityType) {
    ...
    do {
      ...
      print("Error while creating a workout session")
      return
    }
